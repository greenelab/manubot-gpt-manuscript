...
%DIF DEL diffs/v01.1.tex   Tue Dec 19 21:38:46 2023
%DIF ADD diffs/v02.0.tex   Tue Dec 19 21:38:46 2023
...
\textbf{Objective:} Investigate \DIFdelbegin \DIFdel{how models with }\DIFdelend \DIFaddbegin \DIFadd{the use of }\DIFaddend advanced natural language processing \DIFdelbegin \DIFdel{capabilities can }\DIFdelend \DIFaddbegin \DIFadd{models to }\DIFaddend reduce the time-consuming process of writing and revising scholarly manuscripts.\\
...
\DIFdelbegin \DIFdel{It can also help scholars to focus on the most important }\DIFdelend \DIFaddbegin \DIFadd{Our approach also enables scholars to concentrate on critical }\DIFaddend aspects of their work, \DIFdelbegin \DIFdel{such as }\DIFdelend \DIFaddbegin \DIFadd{like }\DIFaddend the novelty of their ideas, \DIFdelbegin \DIFdel{and automate the most tedious parts }\DIFdelend \DIFaddbegin \DIFadd{while automating tedious tasks }\DIFaddend such as adhering to \DIFdelbegin \DIFdel{a certain writing style}\DIFdelend \DIFaddbegin \DIFadd{specific writing styles}\DIFaddend .
...
\DIFdelbegin \DIFdel{We present a novel }\DIFdelend \DIFaddbegin \DIFadd{In this work, we present a human-centric approach for the use of AI in manuscript writing where scholarly text, initially created by humans, is revised through edit suggestions from LLMs, and then ultimately reviewed and approved by humans.
...
2) develop \DIFdelbegin \DIFdel{a workflow }\DIFdelend \DIFaddbegin \DIFadd{an approach }\DIFaddend that integrates these models into the Manubot publishing ecosystem to transparently suggest revisions to improve the quality of the original text.
...
\DIFaddbegin \hypertarget{overview-of-the-manubot-ai-editor}{%
...
Built on this modern and open paradigm, our \DIFdelbegin \DIFdel{AI-based revisionsoftware was developed using GitHub Actions, which }\DIFdelend \DIFaddbegin \DIFadd{Manubot AI Editor (}\url{https://github.com/manubot/manubot-ai-editor}\DIFadd{) includes three components:
...
\DIFadd{The GitHub Actions workflow }\DIFaddend allows the user to easily trigger an automated revision task on the entire manuscript or specific sections of it.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend When the user triggers the action, the manuscript is parsed by section and then by paragraph (Figure \ref{fig:ai_revision}b) \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{that are }\DIFaddend passed to the language model along with a set of custom prompts.
...
\DIFdelbegin \DIFdel{The promptuses the }\DIFdelend \DIFaddbegin \DIFadd{Unless the user specifies a custom prompt, the tool will use a section-specific prompt generator that incorporates the }\DIFaddend manuscript title and keywords, so both must be accurate to obtain the best revision outcomes.
...
\DIFaddbegin \DIFadd{Within the GPT-3 family, }\DIFaddend Davinci models are the most powerful\DIFdelbegin \DIFdel{GPT-3 model}\DIFdelend , whereas Curie ones are less capable but faster and less expensive.
\DIFdelbegin \DIFdel{We mainly focused on the completion endpoint, as the edits endpoint is currently in beta.
}\DIFdelend All models can be fine-tuned using different parameters (see \DIFdelbegin %DIFDELCMD < \href{https://beta.openai.com/docs/api-reference/completions}{%%%
\DIFdelend \DIFaddbegin \href{https://platform.openai.com/docs/api-reference/chat/create}{\DIFaddend OpenAI - API Reference}), and the most important ones can be easily adjusted using our tool.
...
For instance, the context length of Davinci models is 4,000 and 2,048 for Curie (see \DIFdelbegin %DIFDELCMD < \href{https://beta.openai.com/docs/models/overview}{%%%
\DIFdelend \DIFaddbegin \href{https://platform.openai.com/docs/models/gpt-3}{\DIFaddend OpenAI - Models overview}).
\DIFdelbegin \DIFdel{Therefore, it is not possible to use the entire manuscript as input, not even entire sections.
To address this limitation}\DIFdelend \DIFaddbegin \DIFadd{To make sure we never surpass this context length}\DIFaddend , our AI-assisted revision software processes each paragraph of the manuscript with section-specific prompts, as shown in Figure \ref{fig:ai_revision}b.
...
\DIFdelend \DIFaddbegin \href{https://platform.openai.com/tokenizer\%5D}{\DIFaddend OpenAI - Tokenizer}.
...
\DIFaddbegin \DIFadd{Assessing the performance of text generation tasks is challenging, and this is specially true for automatic revisions of scientific content.
...
Model \texttt{text-curie-001} is faster and cheaper than Davinci models, and is defined as ``very capable'' by its authors (see \DIFdelbegin %DIFDELCMD < \href{https://beta.openai.com/docs/models/gpt-3}{%%%
\DIFdelend \DIFaddbegin \href{https://platform.openai.com/docs/models/gpt-3}{\DIFaddend OpenAI - GPT-3 models}).
...
To mitigate this, }\DIFdelend \DIFaddbegin \DIFadd{For the evaluation of our tool, }\DIFaddend we used three manuscripts of our own authorship (Table \ref{tbl:manuscripts}): the Clustermatch Correlation Coefficient (CCC) {[}\protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-eirYTTyk}{10}{]}%%%
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{prompt-engineering}{%
\paragraph{Prompt engineering}\label{prompt-engineering}}
\DIFaddend 

We \DIFdelbegin \DIFdel{found that Davinci models outperformed the Curie modelacross all manuscripts.
The Curie model is faster and less expensive than Davinci models }\DIFdelend \DIFaddbegin \DIFadd{extensively tested our tool, including prompts, using a unit testing framework.
...
We found that adding the instruction }\emph{\DIFadd{``most of the citations to other academic papers are kept''}} \DIFadd{to the prompt was enough to achieve this with the most capable model}\DIFaddend .
\DIFdelbegin \DIFdel{However, the PRs show that the }\DIFdelend \DIFaddbegin \DIFadd{We also wrote unit tests to ensure the models returned citations in the correct Manubot/Markdown format (e.g., }\texttt{{[}\DIFadd{@doi:...}{]}} \DIFadd{or }\texttt{{[}\DIFadd{@arxiv:...}{]}}\DIFadd{), and found that no changes to the prompt were needed for this (i.e., the model automatically detected the correct format in most cases).
...
\DIFadd{Our initial human assessments across the three manuscripts and unit tests revealed that, although faster and the least expensive, the Curie }\DIFaddend model was not able to produce acceptable revisions for any of the manuscripts.
...
%DIFDELCMD < %%%
\DIFdelend The \texttt{text-davinci-003} model produced the best results for all manuscripts and across the different sections.
\DIFdelbegin \DIFdel{Since both }\DIFdelend \DIFaddbegin \DIFadd{This led us to focus on the }\DIFaddend \texttt{text-davinci-003} \DIFdelbegin \DIFdel{and }\texttt{\DIFdel{text-davinci-edit-001}} %DIFAUXCMD
...
All references to scientific literature were kept in the correct Manubot format, although our prompts do not specify the \DIFdelbegin \DIFdel{format of the text}\DIFdelend \DIFaddbegin \DIFadd{references format}\DIFaddend .
...
\DIFdelbegin \DIFdel{However}\DIFdelend \DIFaddbegin \DIFadd{In these cases}\DIFaddend , rerunning the automated revision \DIFdelbegin \DIFdel{should solve this as the model is stochastic}\DIFdelend \DIFaddbegin \DIFadd{might solve this type of issues}\DIFaddend .
...
That paragraph describes Figure 1 of the CCC manuscript {[}\protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-eirYTTyk}{10}{]}%%%
...
\DIFdelbegin \DIFdel{We implemented }\DIFdelend \DIFaddbegin \DIFadd{Our tool, the Manubot AI Editor, implements }\DIFaddend AI-based revision models into the Manubot publishing platform.
...
Although the evaluation of \DIFdelbegin \DIFdel{the revision tool is subjective}\DIFdelend \DIFaddbegin \DIFadd{automatic text revision is challenging}\DIFaddend , we found that most paragraphs were improved\DIFaddbegin \DIFadd{, while in some cases the model removed important information or introduced errors}\DIFaddend .
...
\DIFdelend \DIFaddbegin \hyperlink{ref-S1Lim9f9}{16}{]} \DIFaddend or fine-tuning the model using an additional corpus of academic writing focused on particularly challenging sections.
...
Using chatbots that retain context, such as \href{https://openai.com/blog/chatgpt}{OpenAI's ChatGPT}, could enable the revision of individual paragraphs while considering previously processed text\DIFdelbegin \DIFdel{.
Since an official }\href{https://openai.com/blog/introducing-chatgpt-and-whisper-apis}{\DIFdel{ChatGPT API}} %DIFAUXCMD
\DIFdel{became available recently, }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend we plan to update our workflow to support this strategy.
...
\DIFdel{, or OPT }\DIFdelend \DIFaddbegin \hyperlink{ref-A213xAuD}{19}{]}\DIFadd{, or Mistral 7B }\DIFaddend {[}\protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-nEabcf28}{16}{]}%%%
\DIFdel{, provide similar capabilities}\DIFdelend \DIFaddbegin \hyperlink{ref-1wOalSCp}{20}{]}\DIFadd{, are growing in popularity and capabilities, }\DIFaddend but lack the user-friendly OpenAI API.
