...
%DIF DEL diffs/v03.0.tex   Wed May 22 22:58:10 2024
%DIF ADD diffs/v04.0.tex   Wed May 22 22:58:10 2024
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:llm_judge}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includesvg[width=0.9\textwidth,height=\textheight]{images/llm_judge.svg}
%DIFDELCMD < \caption{\textbf{Automated assessment of preference over revised paragraphs.}
%DIFDELCMD < A revision score (\(y\)-axis) close to 1 indicates that the LLM acting as a judge preferred the revised paragraph over the original one, while a score of -1 indicates the opposite; a zero value indicates either a tie or position bias.
%DIFDELCMD < Each point represents the average score of paragraphs from a section in one of the four manuscripts: CCC, PhenoPLIER, BioChatter and Epistasis.}\label{fig:llm_judge}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \hypertarget{fig:llm_judge}{%
\centering
...
\DIFaddbegin \DIFadd{A revision score above zero indicates that the LLM acting as a judge preferred the revised paragraph over the original one on average, while a score below zero indicates the opposite.
}\DIFaddend It can be seen that the two models used as judges, GPT-3.5 Turbo and GPT-4 Turbo, generally agreed and favored the revised paragraphs over the original ones (revision score above zero) in most cases.
...
\DIFdelbegin \DIFdel{We designed section-specific prompts to guide the revision of text using GPT-3.
...
However, }\DIFdelend \DIFaddbegin \DIFadd{Our approach has some limitations.
We found that }\DIFaddend revising abstracts proved more challenging for the model, as revisions often removed background information about the research problem.
...
\DIFdelbegin \DIFdel{Open }\DIFdelend \DIFaddbegin \DIFadd{Regarding the LLM used, open }\DIFaddend and semi-open models, such as BLOOM {[}\protect\hyperlink{ref-I4d1F0yv}{16}{]}, Meta's Llama 2 {[}\protect\hyperlink{ref-A213xAuD}{17}{]}, and Mistral 7B {[}\protect\hyperlink{ref-1wOalSCp}{18}{]}, are growing in popularity and capabilities, but they lack the user-friendly OpenAI API\DIFaddbegin \DIFadd{.
...
These two models have shown limited self-enhancement bias and high alignment with human preferences }{[}\protect\hyperlink{ref-LhEwBH2w}{13}{]}\DIFadd{.
In this study, we found that the automated assessments were consistent with our human evaluations}\DIFaddend .
