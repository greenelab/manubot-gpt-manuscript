...
The AI-based revision task is based on a machine learning model trained on a corpus of scientific papers from the same field as the manuscript (Figure {@fig:ai_revision}b).
This model is used to identify and suggest revisions to the text.
The output of the revision task is a modified version of the manuscript that is ready for submission.
...
Furthermore, Manubot automates the process of generating a formatted manuscript (such as HTML, PDF, DOCX; Figure {@fig:ai_revision}a).
...
The AI-based revision task is based on a machine learning model trained on a corpus of scientific papers from the same field as the manuscript (Figure {@fig:ai_revision}b).
This model is used to identify and suggest revisions to the text, resulting in a modified version of the manuscript that is ready for submission.
This reduces the time and effort required for manuscript preparation and submission.
...
The model then returns a reformatted version of the text, which is output.
...
We used the following equation to predict the cost of a run:

We used the OpenAI API\cite{OpenAIAPI} for access to these models.
...
$$cost = \frac{length}{1000} \times price$$
{#cost_equation}

where $length$ is the length of the manuscript in words and $price$ is the cost per 1000 words.


### Implementation details

Our tools are comprised of Python scripts that perform the AI-based revision ([https://github.com/greenelab/manubot-ai-editor](https://github.com/greenelab/manubot-ai-editor)) and a GitHub Actions workflow.
The workflow integrates the manuscript with Manubot.
...
Equations definitions (such as '$$ ...
$$ {#id}') are included with newlines before and after, and the most important symbols in equations are defined.
...
A paragraph in the Results section has fewer citations but many references to figures or tables, which must provide enough details about the experiments to understand and interpret the outcomes.
...
However, the language model only processes a single paragraph from a section, potentially losing important context to produce a better output.
Nonetheless, we find that the model still produces high-quality output (see [Results](#sec:results)).
To optimize the output, the maximum number of tokens (parameter `max_tokens`) is set as twice the estimated number of tokens in the paragraph (one token approximately represents four characters, see [OpenAI - Tokenizer](https://beta.openai.com/tokenizer]).
...
To mitigate this, the user can adjust the "sampling temperature" or "nucleus sampling" parameters (we use `temperature=0.5` by default).
...
Alternatively, the user can instruct the model to generate several completions and select the one with the highest log probability per token, which can improve the quality of the revision.
...
The workflow consists of three steps: (1) AI-assisted revision of the manuscript, (2) automatic generation of citations, and (3) automated formatting of equations. 
...
The AI-assisted revision step uses a natural language processing (NLP) model trained on a corpus of academic papers to suggest changes to the manuscript text.
These changes are then reviewed and approved by the authors.
The automatic citation generation step uses a citation recommendation system to suggest appropriate references for the text.
Finally, the automated equation formatting step uses an equation
