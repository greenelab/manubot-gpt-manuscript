...
In PhenoPLIER, the model also in general produced high-quality revisions for `most`{.red} paragraphs, while keeping the Manubot/Markdown format for citations and references to figures, tables and other section of the manuscript.
...
The revised paragraph describes an experiment that does not exist with a reference to a nonexisting section using, however, the correct Markdown format (`[Methods](#sec:methods:coexp)`).
The new text does not mention the CRISPR screen and the gene symbols identified to be associated with the regulation of lipids, which are the key elements in the original text.
Instead, the model seemed to have focused more on the title and keywords of the manuscript (Table @tbl:manuscripts) that are of all prompts (Figure @fig:ai_revision).
It included the idea of a "gene co-expression" analysis (a keywords) to identify "therapeutic targets" (another keyword), and replaced the mention of "sets of genes" in the original text with "clusters of genes" in the revision (closer to the "clustering" keyword).
Instead of considering this as a purely negative example, the output of the model also suggests that the original paragraph is maybe too short and should probably be merged with the next one, which describes follow-up and related experiments.
