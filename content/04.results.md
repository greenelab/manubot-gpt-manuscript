...
These PRs (three per manuscript) can be accessed from the "Pull requests" tab from each repository, where they are titled `GPT (MODEL) used to revise manuscript`, where `MODEL` is the identifier of the model used.
PRs show the differences between the original text and the suggestions by the AI-based revision tool.
